...
DEFAULT_DATABASE_RELATIVE_PATH = os.environ.get(
    "MADAPI_DATABASE_RELATIVE_PATH", "opt/madapi-new-api/data/one-api.db"
)
...
def verify_extracted(root: Path) -> dict[str, object]:
    ...
    database_name = str(manifest.get("database_path") or DEFAULT_DATABASE_RELATIVE_PATH)
    database = (root / database_name).resolve()
    if database != root and root not in database.parents:
        raise RuntimeError(f"database path escapes recovery root: {database_name}")
    if not database.is_file():
        raise RuntimeError(f"recovered database is missing: {database_name}")
