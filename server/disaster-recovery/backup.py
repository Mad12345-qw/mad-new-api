...
FORMAT_VERSION = 1
DEFAULT_DB = Path("/opt/new-api/data/one-api.db")
DEFAULT_LOCAL_DIR = Path("/opt/new-api/backups/disaster-recovery/hourly")
DEFAULT_REPO = Path("/opt/madapi-private-config-backups")
DEFAULT_PUBLIC_KEY = Path("/etc/madapi-backup/recovery-public.pem")
DEFAULT_LOCK = Path("/run/lock/madapi-disaster-recovery.lock")

REQUIRED_FILES = (
    Path("/opt/new-api/.env"),
    Path("/opt/new-api/docker-compose.yml"),
    Path("/etc/nginx/sites-enabled/mad.myddns.me"),
)

OPTIONAL_PATHS = (
    Path("/opt/new-api/cpa-codex"),
    Path("/etc/letsencrypt"),
    Path("/etc/systemd/system/new-api-autoupdate.service"),
    Path("/etc/systemd/system/new-api-autoupdate.timer"),
    Path("/etc/systemd/system/new-api-watchdog.service"),
    Path("/etc/systemd/system/new-api-watchdog.timer"),
    Path("/etc/systemd/system/cpa-codex-autodeploy.service"),
    Path("/etc/systemd/system/cpa-codex-autodeploy.timer"),
    Path("/etc/systemd/system/madapi-local-backup.service"),
    Path("/etc/systemd/system/madapi-local-backup.timer"),
    Path("/etc/systemd/system/madapi-offsite-backup.service"),
