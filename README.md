...
- token-scoped Codex Desktop setup for users already signed in with ChatGPT OAuth, with Windows/macOS scripts that reject API-key-only or unsigned-in states before changing files, preserve OAuth identity, provider identity, sessions, and unrelated configuration, configure direct MadAPI bearer authentication, clear only the stale model cache and static catalog setting, use a 360-second stream timeout and three request retries, and create a rollback backup before replacing the configuration
