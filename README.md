...
- isolated Codex compatibility surfaces at `/codex/v1` and `/codex/cockpit/v1`: OAuth sessions use the complete CPA-native catalog, while API Key sessions use a stable eight-model Cockpit projection; both retain NewAPI authentication, channel selection, retry, billing, and settlement
- a private request-scoped CPA executor behind NewAPI's selected-channel boundary: xAI, Claude, Gemini, Kimi, and Codex channels use CPA's native provider executors, while generic channels use CPA's OpenAI compatibility executor; the private dispatcher is authenticated, internal-only, and never sits in front of NewAPI
