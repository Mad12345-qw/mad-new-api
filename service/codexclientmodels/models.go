// Package codexclientmodels builds model catalogs for official Codex clients.
package codexclientmodels
...
// ModelInfo is the MadAPI adapter for the pinned Codex registry metadata.
...
type ModelInfoLookupFunc func(string) *ModelInfo
type SearchSupportFunc func(string) bool
...
func BuildResponse(availableModels []map[string]any, providersForModel ProvidersForModelFunc, optimizeMultiAgentV2 bool, lookupModelInfo ModelInfoLookupFunc, supportsSearch SearchSupportFunc) map[string]any {
	return map[string]any{
		"models": buildCodexClientModels(availableModels, providersForModel, optimizeMultiAgentV2, lookupModelInfo, supportsSearch),
	}
}

func buildCodexClientModels(models []map[string]any, providersForModel ProvidersForModelFunc, optimizeMultiAgentV2 bool, lookupModelInfo ModelInfoLookupFunc, supportsSearch SearchSupportFunc) []map[string]any {
	...
	for _, model := range models {
		...
		applyCodexClientModelMetadata(entry, id, model, optimizeMultiAgentV2, lookupModelInfo)
		applyCodexClientSearchToolSupport(entry, id, false, providersForModel, supportsSearch)
		sanitizeCodexClientReasoningMetadata(entry)
		applyCodexClientVisibilityOverride(entry, id)
		result = append(result, entry)
	}
...
func loadCodexClientModelTemplates() (map[string]map[string]any, map[string]any, error) {
	return loadCodexClientModelTemplatesSnapshot(codexClientModelsJSON, 1)
}
...
func applyCodexClientModelMetadata(entry map[string]any, id string, model map[string]any, optimizeMultiAgentV2 bool, lookupModelInfo ModelInfoLookupFunc) {
	var info *ModelInfo
	if lookupModelInfo != nil {
		info = lookupModelInfo(id)
	}
...
func applyCodexClientThinkingMetadata(entry map[string]any, thinking *ThinkingSupport) {
