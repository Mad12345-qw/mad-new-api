...
- isolated Codex compatibility surfaces at `/codex/v1` and `/codex/cockpit/v1`: the Cockpit route projects each token's current text-model access into the Codex Desktop catalog, while Responses requests retain NewAPI authentication, channel selection, mapping, retry, billing, and settlement
...
- one token-scoped Codex Desktop setup for OAuth, API Key, and unsigned users: Windows/macOS scripts route every user through `/codex/cockpit/v1`, refresh the MadAPI model catalog dynamically, preserve every existing `auth.json` byte-for-byte, leave `auth.json` absent for new users so Codex presents its native OAuth/API Key chooser, preserve provider identity, sessions, and unrelated configuration, clear stale model cache and third-party catalog settings, use a 360-second stream timeout and three request retries, and back up configuration before replacement
