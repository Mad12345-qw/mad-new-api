...
- two complete CPA front proxies for Codex execution: native and Cockpit requests traverse CPA's original HTTP/WebSocket handlers, scheduler, retries, translators, stream finalization, and error recovery, then forward the original MadAPI bearer token to NewAPI for authentication, channel selection, billing, and settlement; each selected model/channel pair carries an explicit wire protocol so native Responses, Claude Messages, Gemini, xAI, and OpenAI-compatible aggregators use the matching CPA executor without changing channel weights or billing
