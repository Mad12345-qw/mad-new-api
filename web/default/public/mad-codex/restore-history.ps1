...
if (Test-Path -LiteralPath $sessionsPath) {
    foreach ($file in @(Get-ChildItem -LiteralPath $sessionsPath -Recurse -Filter 'rollout-*.jsonl' -File -ErrorAction SilentlyContinue)) {
        ...
        $timestampProperty = $meta.payload.PSObject.Properties['timestamp']
        if ($null -ne $timestampProperty -and -not [string]::IsNullOrWhiteSpace([string]$timestampProperty.Value)) {
            [void][datetime]::TryParse([string]$timestampProperty.Value, [ref]$timestamp)
        }
        $records.Add([pscustomobject]@{ Id = [string]$meta.payload.id; Cwd = [string]$meta.payload.cwd; UpdatedAt = $timestamp.ToUniversalTime().ToString('o'); SortAt = $timestamp })
    }
}
