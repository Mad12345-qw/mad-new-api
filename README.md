...
- isolated Codex compatibility surfaces at `/codex/v1` and `/codex/cockpit/v1`: OAuth sessions use the complete native catalog, while API Key sessions use a stable eight-model Cockpit projection; model catalogs remain token-scoped in NewAPI
- one NewAPI-first selected-channel CPA runtime for Codex execution: NewAPI authenticates, bills, applies weights, and selects the channel before the pinned MadAPI CPA fork runs the matching native Responses, Claude Messages, Gemini, xAI, or OpenAI-compatible executor; HTTP and WebSocket requests share the same translators, stream finalization, retry boundaries, and original bearer-token settlement path without a dynamic plugin or dual CPA containers
...
- CPA-compatible Responses WebSocket support remains on the same NewAPI compatibility controller and submits every generated turn through the existing billed `/v1/responses` route
- one token-scoped Codex Desktop setup for OAuth, API Key, and unsigned users: Windows/macOS scripts preserve every existing `auth.json` byte-for-byte, leave it absent for new users so Codex presents its native login chooser, select `/codex/v1` for OAuth/unconfigured sessions and `/codex/cockpit/v1` for API Key sessions, re-evaluate that choice on every catalog refresh, preserve provider identity, sessions, and unrelated configuration, clear stale model cache and third-party catalog settings, use a 360-second stream timeout and three request retries, and back up configuration before replacement; both platforms refresh once during installation and silently at user login without a recurring timer

GitHub Actions checks out a pinned commit from the MadAPI CPA fork, verifies formatting and runtime tests without applying a patch chain, runs backend, frontend, and image compatibility tests, builds the Docker image away from the production server, and publishes stable release artifacts. No production credentials are stored in this repository.
