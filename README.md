...
- token-scoped Codex Desktop setup from the API key page, with Windows/macOS scripts that preserve the existing provider identity, sessions, and unrelated configuration; configure direct bearer authentication, clear only the stale model cache and static catalog setting, use a 360-second stream timeout and three request retries, and create a rollback backup before replacing the configuration
