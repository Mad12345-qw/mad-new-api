...
Assert-True ($refreshSource.Contains('/mad-codex/codex-model-templates.json')) 'Self-hosted Codex model template is missing.'
...
try {
    ...
    Assert-True ($result.Contains('base_url = "http://127.0.0.1:13016/codex/v1"')) 'Dedicated Codex route is missing.'
    ...
    Assert-True (([regex]::Matches($result, '(?m)^localeOverride = "zh-CN"\r?$')).Count -eq 1) 'Chinese locale override is duplicated or nested.'
    Assert-True ($result.IndexOf('localeOverride = "zh-CN"') -lt $result.IndexOf('[')) 'Chinese locale override is not a root key.'
    ...
    Assert-True (([regex]::Matches($repeat, '(?m)^localeOverride = "zh-CN"\r?$')).Count -eq 1) 'API Chinese locale override is duplicated or nested.'
    Assert-True ($repeat.IndexOf('localeOverride = "zh-CN"') -lt $repeat.IndexOf('[')) 'API Chinese locale override is not a root key.'
...
& $refreshScript -CodexHome $codexHome | Out-Host
    ...
    Assert-True ('gpt-5.3-codex' -in $slugs) 'Sol Pro compatibility slug is missing.'
    Assert-True ('gpt-5.2' -in $slugs) 'Terra Pro compatibility slug is missing.'
