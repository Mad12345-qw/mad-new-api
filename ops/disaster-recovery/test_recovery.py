...
class RecoveryTests(unittest.TestCase):
    def test_optional_paths_do_not_include_removed_media_or_cpa_components(self):
        optional_names = [str(path).lower() for path in backup.OPTIONAL_PATHS]
        self.assertFalse(any("cpa" in path or "image-media" in path for path in optional_names))
    ...
    def test_restore_uses_database_path_from_manifest(self):
        import json
        import sqlite3

        with tempfile.TemporaryDirectory() as temp_name:
            root = Path(temp_name)
            database = root / "opt/madapi-new-api/data/one-api.db"
            ...
            manifest = {
                "format_version": 1,
                "database_path": "opt/madapi-new-api/data/one-api.db",
                "files": [],
            }
            ...
            result = restore.verify_extracted(root)
            self.assertEqual(result["database_path"], "opt/madapi-new-api/data/one-api.db")
