...
This directory defines an independent NewAPI backup and recovery component. It
does not start NewAPI, CPA, media gateways, Nginx, or any production service.
The public source repository never receives plaintext backup data.
...
The default archive contains only the configured NewAPI database, Compose file,
and environment file. Extra paths must be explicitly listed in `MADAPI_RECOVERY_OPTIONAL_PATHS`; CPA, media gateway, Nginx, and deployment
artifacts are not included by default. It does not contain host SSH credentials.
...
```sh
python3 restore.py /path/to/bundle --private-key /secure/recovery-private.pem --output /secure/recovery-output
```
