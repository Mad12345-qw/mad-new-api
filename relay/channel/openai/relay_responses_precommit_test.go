...
func responsesPrecommitContext(t *testing.T, internal bool) (*gin.Context, *httptest.ResponseRecorder) {
	t.Helper()
	...
	if internal {
		c.Request.Header.Set("X-MadAPI-Codex-Compat", relayconvert.CodexResponsesInternalMarker())
	}
	return c, recorder
}
...
func responsesPrecommitInfo() *relaycommon.RelayInfo {
	return &relaycommon.RelayInfo{
		IsStream:                      true,
		RequireResponsesTerminalEvent: true,
...
func TestCodexResponsesNamedErrorWithoutPayloadTypeStaysUncommitted(t *testing.T) {
	c, recorder := responsesPrecommitContext(t, true)
	_, newAPIError := OaiResponsesStreamHandler(c, responsesPrecommitInfo(), responsesHTTPResponse(
		`event: error
data: {"error":{"type":"server_error","code":"upstream_failed","message":"temporary upstream failure"}}
