...
// inspectCodexResponsesFirstEvent keeps a single-layer Codex Responses stream
// uncommitted until the first upstream data event is known to be usable. A
// failure or empty stream is returned to NewAPI's existing retry loop; a valid
// first event is replayed byte-for-byte before normal streaming continues.
