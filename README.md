...
- an isolated Codex Desktop compatibility surface at `/codex/v1`: its catalog is synthesized from each token's current conversation-model access, while Responses requests retain NewAPI authentication, channel selection, mapping, retry, billing, and settlement
...
- token-scoped Codex Desktop setup for both ChatGPT OAuth and API Key users, with Windows/macOS scripts that preserve OAuth identity when present, create the official API Key auth projection otherwise, preserve provider identity, sessions, and unrelated configuration, select exactly one compatible provider authentication path, refresh the MadAPI model catalog dynamically, clear stale model cache and third-party static catalog settings, use a 360-second stream timeout and three request retries, and create rollback backups before replacing authentication or configuration files
