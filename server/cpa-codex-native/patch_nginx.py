...
BEGIN_MARKER = "# MadAPI native CPA Codex routes begin"
END_MARKER = "# MadAPI native CPA Codex routes end"
OLD_BEGIN_MARKER = "# MadAPI CPA Codex front routes begin"
OLD_END_MARKER = "# MadAPI CPA Codex front routes end"
...
def remove_marked_range(source: str, begin_marker: str, end_marker: str) -> str:
...
def reconcile_routes(source: str) -> str:
    cleaned = remove_legacy_route(source)
    cleaned = remove_marked_range(cleaned, OLD_BEGIN_MARKER, OLD_END_MARKER)
    cleaned = remove_marked_range(cleaned, BEGIN_MARKER, END_MARKER)
    for location in ("location ^~ /codex/v1/", "location ^~ /codex/cockpit/v1/"):
...
def main() -> int:
    ...
    backup = BACKUP_DIR / f"mad.myddns.me.before-native-cpa-{datetime.utcnow():%Y%m%d-%H%M%S}"
