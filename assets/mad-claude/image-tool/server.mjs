...
function mimeTypeFor(data, headerValue = '') {
  ...
  if (['image/png', 'image/jpeg', 'image/webp', 'image/gif'].includes(declared)) {
    return declared
  }
...
async function downloadImage(imageUrl) {
  ...
  if (fixturePath) {
    ...
    if (data.length > MAX_IMAGE_BYTES) throw new Error('fixture image exceeds 20 MB')
  ...
  if (declaredLength > MAX_IMAGE_BYTES) throw new Error('generated image exceeds 20 MB')
  const data = Buffer.from(await response.arrayBuffer())
  if (data.length > MAX_IMAGE_BYTES) throw new Error('generated image exceeds 20 MB')
  return { mimeType: mimeTypeFor(data, response.headers.get('content-type')), data }
}

function imageIdFor(prompt, size) {
  return createHash('sha256').update(`${prompt}\0${size}`, 'utf8').digest('hex')
}
...
function writeMetadata(imageId, metadata) {
  ...
  fs.writeFileSync(temporary, JSON.stringify(metadata), { encoding: 'utf8', mode: 0o600 })
...
function markPending(imageId) {
  ...
  writeMetadata(imageId, { id: imageId, state: 'pending', createdAt: Date.now() })
}

function markFailed(imageId) {
  writeMetadata(imageId, { id: imageId, state: 'failed', createdAt: Date.now() })
}
...
function cleanCache() {
  ...
  for (const name of fs.readdirSync(directory)) {
    ...
    if (!name.endsWith('.json')) files.push({ filePath, size: stat.size, mtimeMs: stat.mtimeMs })
  }
...
function cacheImage(imageId, mimeType, data) {
  const target = path.join(cacheDirectory(), `${imageId}${extensionFor(mimeType)}`)
  const temporary = `${target}.${process.pid}.tmp`
...
function readCachedImage(imageId) {
  ...
  if (path.dirname(imagePath) !== path.resolve(directory)) throw new Error('invalid cached image path')
  const data = fs.readFileSync(imagePath)
  if (data.length > MAX_IMAGE_BYTES) throw new Error('cached image exceeds 20 MB')
  return { mimeType: mimeTypeFor(data, metadata.mimeType), data }
}

async function requestGeneration(prompt, size) {
  const fixtureResponsePath = process.env.MADAPI_IMAGE_TEST_RESPONSE_JSON
  if (fixtureResponsePath) return JSON.parse(fs.readFileSync(fixtureResponsePath, 'utf8'))
  ...
  if (!response.ok) throw new Error(`MadAPI returned HTTP ${response.status}: ${body.slice(0, 1000)}`)
...
function readResource(uri) {
      ...
      contents: [
        {
          ...
          text: fs.readFileSync(new URL('./widget.html', import.meta.url), 'utf8'),
  ...
  if (uri.startsWith(IMAGE_URI_PREFIX)) {
    const { mimeType, data } = readCachedImage(uri.slice(IMAGE_URI_PREFIX.length))
    return {
...
async function handle(message) {
  ...
  if (method === 'tools/call') {
    if (message.params?.name !== 'generate_image') throw new Error('unknown tool')
    return {
      jsonrpc: '2.0',
      id,
  ...
  if (method === 'resources/read') {
    return { jsonrpc: '2.0', id, result: readResource(String(message.params?.uri || '')) }
  }
