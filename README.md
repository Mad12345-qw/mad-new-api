...
The operator now supplies only a third-party API base URL and key. The detector queries OpenAI- and Anthropic-compatible model lists, keeps GPT and Claude text models plus only `gpt-image-2`, and assigns a route per selected model from advertised endpoint capabilities. Claude uses Anthropic Messages or an explicitly reported OpenAI translation route; current GPT models prefer Responses with a guarded Chat fallback; `gpt-image-2` uses non-generation validation probes against the Images contract. Gemini and every other image model are currently excluded from provenance runs.
